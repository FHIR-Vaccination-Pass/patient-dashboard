...
export function VaccineDetailPage() {
    const vaccination = {
        ...
        description: 'There are several COVID-19 vaccines validated for use by WHO (given Emergency Use Listing).\n' +
            ...
            '                        to WHO approval.',
        vaccines: [
            {name: 'Corminaty', manufacturer: 'BionTech', numberOfDoses: 3, recommendedAgeStart: 0, recommendedAgeEnd: 99},
            {name: 'AstraZeneca', manufacturer: 'AstraZeneca', numberOfDoses: 3, recommendedAgeStart: 40, recommendedAgeEnd: 99},
            {name: 'Jhonnson & Jhonsson', manufacturer: 'Jhonnson & Jhonsson', numberOfDoses: 1, recommendedAgeStart: 0, recommendedAgeEnd: 99},
            {name: 'Moderna', manufacturer: 'Moderna', numberOfDoses: 3, recommendedAgeStart: 0, recommendedAgeEnd: 99},
        ],
        relevantLocations: ['Global']
    };
    return(
        <VStack>
            <Text fontSize={'lg'} align={'left'}>{ vaccination.name }</Text>
            <HStack>
                <div className={'vaccination-description'}>
                    <span><span className={'accordion-content-header'}>Disease:</span> { vaccination.name }<br/></span>
                    <span><span className={'accordion-content-header'}>Immunization against:</span> { vaccination.immunizationAgainst }<br/></span>
                    <span><span className={'accordion-content-header'}>Relevant Locations:</span> { vaccination.relevantLocations }<br/></span>
                    <span><span className={'accordion-content-header'}>Description:</span> { vaccination.description }<br/></span>
                    <span className={'accordion-content-header'}>Available vaccines:</span><br/>
                </div>
            </HStack>
            <div style={{position: 'relative', width: '100%'}}>
                <Accordion style={{position: 'absolute', left: 0, width: '100%'}} defaultIndex={[]} allowMultiple>
                    {vaccination.vaccines.map( vaccine => (
                        <AccordionItem>
                            ...
                            <AccordionPanel pb={4}>
                                <HStack>
                                    <div>
                                        <span><span className={'accordion-content-header'}>Manufacturer:</span> { vaccine.manufacturer }<br/></span>
                                        <span><span className={'accordion-content-header'}>Number of doses:</span> { vaccine.numberOfDoses }<br/></span>
                                        <span><span className={'accordion-content-header'}>Recommended age:</span> { vaccine.recommendedAgeStart } - { vaccine.recommendedAgeEnd }<br/></span>
                                    </div>
                                </HStack>
                            </AccordionPanel>
                        </AccordionItem>
